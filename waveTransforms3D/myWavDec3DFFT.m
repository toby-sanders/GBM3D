function C = myWavDec3DFFT(Psi,levels,U)
...
for i = 1:levels % loop for computing detail coefficients
    ...
    FU = fftn(U);
    for j = 1:7
        C{i,j} = ifftn(FU.*fftn(Psi(:,:,:,j+1),[size(U,1),size(U,2),size(U,3)]));
        C{i,j} = real(C{i,j}(1:2:end,1:2:end,1:2:end));
    end
    % low pass filter, downsample, and move to next level
    % U = circshift(imfilter(U,Psi(:,:,:,1),'conv','circular'),[Ny/2,Nx/2,Nz/2]);
    U = real(ifftn(FU.*fftn(Psi(:,:,:,1),[size(U,1),size(U,2),size(U,3)])));
