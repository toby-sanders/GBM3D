...
for i = 1:levels
    for j = 1:7
      ...
      %  Cup{i,j} = circshift(imfilter(Cup{i,j},Psi(:,:,:,j+1),'corr','circular'),[-Ny/2+1,-Nx/2+1,-Nz/2+1]);
        Cup{i,j} = ifftn(fftn(Cup{i,j}).*conj(fftn(Psi(:,:,:,j+1),[size(Cup{i,j},1),size(Cup{i,j},2),size(Cup{i,j},3)])));
    end
end
...
for i = 1:levels
    ...
    for k = i+1:levels+1
        tmp = zeros(2*size(Cup{k,1},1),2*size(Cup{k,1},2),2*size(Cup{k,1},3));
