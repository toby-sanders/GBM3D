function [U,out] = denoiseAndAggregateV(Y,Volume,I,S,sigma,opts)
...
U = zeros(m+d,n+d,K); % pad for block matches that wrap around
out.K = zeros(m+d,n+d,K);
out.Z = zeros(m,n);
...
for y = 1:M
    for x = 1:N
        ...
        % weight = 1;
        for i = 1:Nm
            [a,b,c] = ind2sub([d,d,K],S{y,x}(i));
            a = a + (y-3/2)*d;
            b = b + (x-3/2)*d;
            if a>0 & b>0
            U(a:a+d-1,b:b+d-1,c) = U(a:a+d-1,b:b+d-1,c) + weight*blocks(:,:,i);
            out.K(a:a+d-1,b:b+d-1,c) = out.K(a:a+d-1,b:b+d-1,c) + weight;
            end
        end
