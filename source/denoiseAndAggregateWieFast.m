...
V1 = pagemtimes(T,V1);
V1 = pagemtimes(V1,T');
% V1 = pagetranspose(pagemtimes(T,'none',V1,'transpose'));
...
V2 = pagemtimes(T,V2);
V2 = pagemtimes(V2,T');
% V2 = pagetranspose(pagemtimes(T,'none',V2,'transpose'));
V2 = myWavDec1D(Psi,Phi,levels,V2,3);
V2 = V2.*filt;

% inverse transform to obtain denoise blocks
V2 = myWavRec1D(Psi,Phi,levels,V2,3);
V2 = pagemtimes(V2,Tinv');
% V2 = pagetranspose(pagemtimes(Tinv,'none',V2,'transpose'));
V2 = pagemtimes(Tinv,V2);
V2 = max(V2,0);
