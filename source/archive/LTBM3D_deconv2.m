...
LTopts.cycleSpin = 2;
LTopts.matchSpins =  [0 4 sZ/2-1];
out.U0 = LTBM3D(out.recWei1,out.sigmaPSD,LTopts);

imagePSD = abs(fft2(out.U0)).^2;
filt2 = conj(hhat).*imagePSD./(imagePSD.*hhat2 + 10*sigma^2*opts.lambda*V);
out.recWei2 = real(ifft2(filt2.*FI));
out.sigmaPSD2 = sigma^2*abs(filt2).^2;
LTopts.cycleSpin = 2;
LTopts.matchSpins =  [0 4 sZ/2-1];
U = LTBM3D(out.recWei2,out.sigmaPSD2,LTopts);