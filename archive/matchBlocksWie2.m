...
blockNorms = real(ifft2(conj(fft2(g)).*(fft2(In.^2))));
blockNormsPad = real(ifft2(conj(fft2(g,m+d2,n+d2)).*fft2(Ipad2.^2)));
...
for i = 1:M
    ...
    for j = 1:N
        ...
        xR = In((i-1)*d+1:i*d,(j-1)*d+1:j*d); % reference block
        ...
        blockDistances = real(ifft2(conj(fft2(xR,d+d2,d+d2)).*fft2(Itmp2)));       
