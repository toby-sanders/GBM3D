function [U,out] = denoiseBlocks(I,S,sigma,opts)
...
for y = 1:A
    for x = 1:B
       % fprintf('denoising block %i of %i\n',(y-1)*B + x,A*B);
        ...
        Nm = numel(S{y,x});
        if Nm>1
            Nm2power = ceil(log(Nm)/log(2));
            cellInd = Nm2power - 3;
            % gather all the matched blocks into 3D array
            % blocks = zeros(d,d,Nm,'gpuArray'); 
            for i = 1:Nm
                [a,b] = ind2sub([m,n],S{y,x}(i));
                blocks{cellInd}(:,:,i) = Ipad(a:a+d-1,b:b+d-1);
            end
            padSize = 2^(ceil(log(Nm)/log(2))) - Nm;
            blocks{cellInd}(:,:,i+1:end) = blocks(:,:,1:padSize);
           ...
           %  blocks = [blocks,fliplr(blocks);flipud(blocks),fliplr(flipud(blocks))];
            ...
            rec = myWavRec3D_GPU(Psi,levels,C);
